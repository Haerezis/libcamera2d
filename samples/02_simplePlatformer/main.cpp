  ...
  _playerSize(tileSize)
{
  ...
  auto res = SDL_CreateWindowAndRenderer(
           _windowWidth,
           _windowHeight,
           SDL_WINDOW_SHOWN,
           &_window,
  ...
  _camera.reset(new libcamera2d::Camera_staticOffsetFromTarget(
      (_windowWidth - _playerSize) / 2,
      (_windowHeight - _playerSize) / 2,
      _windowWidth,
      _windowHeight,
      _tileMapWidth * _tileSize,
      _tileMapHeight * _tileSize));
...
{
  _camera->update(_playerPositionX, _playerPositionY);
}
